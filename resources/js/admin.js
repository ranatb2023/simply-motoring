document.addEventListener('DOMContentLoaded', function () {
    ...
    function renderConnectedCalendars(calendars, settings) {
        ...
        // --- SECTION 2: Calendar to add events to (per service) ---
        ...
        eventsHeader.style.marginBottom = '4px';
        eventsHeader.style.marginTop = '0';
        listContainer.appendChild(eventsHeader);

        const eventsDesc = document.createElement('p');
        eventsDesc.innerText = "Choose which Google Calendar each service's bookings are added to.";
        eventsDesc.style.cssText = 'font-size:0.875rem; color:#6b7280; margin-top:0; margin-bottom:16px;';
        listContainer.appendChild(eventsDesc);

        // Build calendar <option> markup grouped by connected account.
        const buildCalOptions = (selectedEmail, selectedCalId) => {
            let html = '<option value="">— Do not add to a calendar —</option>';
            accountEmails.forEach(email => {
                const cals = groups[email] || [];
                html += `<optgroup label="${email}">`;
                cals.forEach(c => {
                    const calName = c.summaryOverride || c.summary || c.id;
                    const val = email + '||' + c.id;
                    const sel = (email === selectedEmail && c.id === selectedCalId) ? ' selected' : '';
                    html += `<option value="${val}"${sel}>${calName}</option>`;
                });
                html += '</optgroup>';
            });
            return html;
        ...
        mapCard.className = 'gbs-cal-settings-card';
        mapCard.style.cssText = 'padding:0; overflow:hidden; width:50%;';

        if (!services.length) {
            mapCard.innerHTML = '<div style="padding:16px; color:#6b7280; font-size:0.85rem;">No active services to map.</div>';
        }

        services.forEach((svc, idx) => {
            const current = (settings && settings[svc.id]) ? settings[svc.id] : null;
            const curEmail = current ? (current.account_email || '') : '';
            const curCalId = current ? (current.google_calendar_id || '') : '';

            const row = document.createElement('div');
            row.style.cssText = 'display:flex; justify-content:space-between; align-items:center; gap:12px; padding:14px 16px; background:#fff;';
            if (idx < services.length - 1) row.style.borderBottom = '1px solid #e5e7eb';

            row.innerHTML = `
                <span style="font-weight:600; color:#1f2937; font-size:0.875rem;">${svc.name}</span>
                <div style="display:flex; align-items:center; gap:8px;">
                    <select class="gbs-svc-cal-select" data-service-id="${svc.id}" style="border:1px solid #e5e7eb; border-radius:8px; padding:8px 10px; font-size:0.8rem; min-width:220px; background:#fff; color:#374151;">
                        ${buildCalOptions(curEmail, curCalId)}
                    </select>
                    <span class="gbs-svc-cal-status" style="font-size:0.85rem; width:16px; text-align:center;"></span>
                </div>
            `;
            mapCard.appendChild(row);
        });

        listContainer.appendChild(mapCard);

        // Save each service's calendar choice on change
        mapCard.querySelectorAll('.gbs-svc-cal-select').forEach(sel => {
            sel.addEventListener('change', (e) => {
                const serviceId = e.target.dataset.serviceId;
                const val = e.target.value;
                let email = '', calendarId = '';
                if (val) {
                    const parts = val.split('||');
                    email = parts[0];
                    calendarId = parts.slice(1).join('||');
                }
                const statusEl = e.target.parentElement.querySelector('.gbs-svc-cal-status');
                if (statusEl) statusEl.innerHTML = '<span style="color:#2563eb;">…</span>';
                e.target.disabled = true;

                fetch(root + 'google/settings', {
                    method: 'POST',
                    headers: headers,
                    body: JSON.stringify({ service_id: serviceId, account_email: email, calendar_id: calendarId })
                }).then(r => r.json()).then(res => {
                    e.target.disabled = false;
                    if (res.success) {
                        if (statusEl) statusEl.innerHTML = '<span style="color:#059669;">✓</span>';
                        gbsShowToast('Calendar mapping saved');
                        setTimeout(() => { if (statusEl) statusEl.innerHTML = ''; }, 2000);
                    } else {
                        if (statusEl) statusEl.innerHTML = '<span style="color:#ef4444;">!</span>';
                        gbsShowToast('Error saving mapping');
                    }
                }).catch(() => {
                    e.target.disabled = false;
                    if (statusEl) statusEl.innerHTML = '<span style="color:#ef4444;">!</span>';
